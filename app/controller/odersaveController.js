const Tutorial = require("../models/odersaveModel.js");
...
exports.months = (req, res) => {
    ...
    Tutorial.months(req.params.id, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                res.status(404).send({
...
exports.year = (req, res) => {
    ...
    Tutorial.year(req.params.id, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                res.status(404).send({
                    message: `Not found Cabin with id ${req.params.id}.`
...
exports.findbyNgay = (req, res) => {
    Tutorial.findbyNgay(req.params.id, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                res.send([]);
            } else {
